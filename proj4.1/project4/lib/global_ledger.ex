defmodule Project4.GlobalLedger do
    ...
    def get_ledger_data(pid) do
        GenServer.call(pid, {:get_ledger_data})
    end
    ...
    def transfer(pid, from, to, value) do
        GenServer.cast(pid, {:transfer, from, to, value})
    end
    ...
    def get_balance_for_user(pid, public_address) do
        GenServer.call(pid, {:get_balance_for_user, public_address})
    end
    ...
    def handle_cast({:transfer, from, to, value}, {blockchain, outputs, transactions, unverified_transactions}) do
        # IO.inspect {"---->transfering", from, to, value}
        ...
        # IO.inspect {"-------------------"}
        # IO.inspect {input_indices, change, is_possible}
        if is_possible do
    ...
    def handle_call({:get_ledger_data}, _, {blockchain, outputs, transactions, unverified_transactions}) do
        # IO.inspect {:get_ledger_data}
    ...
    def handle_call({:update_ledger_blockchain, public_address, new_blockchain, new_transactions, new_outputs}, _, {blockchain, outputs, transactions, unverified_transactions}) do
        {blockchain, outputs, transactions, unverified_transactions, success} = 
            if length(new_blockchain) == length(blockchain) + 1 and length(unverified_transactions) > 0 do
    ...
    def handle_call({:get_balance_for_user, public_address}, _, {blockchain, outputs, transactions, unverified_transactions}) do
        balance = get_balance_for_user(blockchain, outputs, unverified_transactions, public_address)
    ...
    def get_unspent_outputs_for_user(blockchain, outputs, unverified_transactions, public_address) do
        ...
        # IO.inspect {"output_indices", output_indices, "input_indices", input_indices}
    ...
    def get_balance_for_user(blockchain, outputs, unverified_transactions, public_address) do
        unspent_output_indices = get_unspent_outputs_for_user(blockchain, outputs, unverified_transactions, public_address)
